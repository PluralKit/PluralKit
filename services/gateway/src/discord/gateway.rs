...
use twilight_gateway::{
    create_iterator, error::ReceiveMessageError, ConfigBuilder, Event, EventTypeFlags, Message,
    Shard, ShardId,
};
...
pub async fn runner(
    ...
    while let Some(item) = shard.next().await {
        ...
        // for now only do commands
        if let Event::MessageCreate(evt) = event
            && evt.content.starts_with("pk;")
        {
