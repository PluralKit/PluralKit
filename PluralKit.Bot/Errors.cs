...
public static class Errors
{
    ...
    public static PKError LegacyAlreadyHasProxyTag(ProxyTag requested, PKMember member, Context ctx, string prefix) => new(
        $"This member already has more than one proxy tag set: {member.ProxyTagsString()}\nConsider using the {$"{prefix}member {member.Reference(ctx)} proxy add {requested.ProxyString}".AsCode()} command instead.");
