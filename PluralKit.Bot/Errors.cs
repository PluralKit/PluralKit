...
namespace PluralKit.Bot {
    ...
    public static class Errors {
        ...
        public static PKError NoSystemError(string CommandPrefix) => new PKError($"You do not have a system registered with PluralKit. To create one, type `{CommandPrefix}system new`.");
        public static PKError ExistingSystemError(string CommandPrefix) => new PKError($"You already have a system registered with PluralKit. To view it, type `{CommandPrefix}system`. If you'd like to delete your system and start anew, type `{CommandPrefix}system delete`, or if you'd like to unlink this account from it, type `{CommandPrefix}unlink`.");
        ...
        public static PKError AccountInOtherSystem(string CommandPrefix, PKSystem system) => new PKError($"The mentioned account is already linked to another system (see `{CommandPrefix}system {system.Hid}`).");
        public static PKError UnlinkingLastAccount(string CommandPrefix) => new PKError($"Since this is the only account linked to this system, you cannot unlink it (as that would leave your system account-less). If you would like to delete your system, use `{CommandPrefix}system delete`.");
        ...
        public static PKError LegacyAlreadyHasProxyTag(string CommandPrefix, ProxyTag requested, PKMember member) => new PKError($"This member already has more than one proxy tag set: {member.ProxyTagsString()}\nConsider using the {$"{CommandPrefix}member {member.Reference()} proxy add {requested.ProxyString}".AsCode()} command instead.");
        public static PKError EmptyProxyTags(string CommandPrefix, PKMember member) => new PKError($"The example proxy `text` is equivalent to having no proxy tags at all, since there are no symbols or brackets on either end. If you'd like to clear your proxy tags, use `{CommandPrefix}member {member.Reference()} proxy clear`.");
