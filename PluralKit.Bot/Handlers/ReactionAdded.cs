...
public class ReactionAdded: IEventHandler<MessageReactionAddEvent>
{
    ...
    private async ValueTask HandleProxyDeleteReaction(MessageReactionAddEvent evt, PKMessage msg)
    {
        ...
        // todo: move this into repo somewhere
        var isSameSystem = await _db.Execute(c => c.QuerySingleOrDefaultAsync<bool>(
            "select true from accounts, members where members.id = @member and accounts.uid = @account and members.system = accounts.system",
            new { member = msg.Member, account = evt.UserId }
        ));
    ...
    private async ValueTask HandlePingReaction(MessageReactionAddEvent evt, FullMessage msg)
    {
        ...
        // todo: get system config from member?
