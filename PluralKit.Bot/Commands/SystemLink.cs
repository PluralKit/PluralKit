...
namespace PluralKit.Bot
{
    public class SystemLink
    {
        ...
        public async Task LinkSystem(Context ctx)
        {
            ...
            if (await MiscUtils.EnsureReactionPermissions(ctx, "ask for confirmation", true)) if (!await ctx.PromptYesNo($"{account.Mention}, please confirm the link by clicking the {Emojis.Success} reaction on this message.", user: account)) throw Errors.MemberLinkCancelled;
        ...
        public async Task UnlinkAccount(Context ctx)
        {
            ...
            if (!await ctx.PromptYesNo($"Are you sure you want to unlink <@{id}> from your system?")) throw Errors.MemberUnlinkCancelled;
