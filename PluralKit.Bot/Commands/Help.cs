...
public class Help
{
    ...
    public Task HelpRoot(Context ctx)
        => ctx.Rest.CreateMessage(ctx.Channel.Id, new MessageRequest
        {
            ...
            Embeds = new[] { helpEmbed with { Description = helpEmbed.Description,
                                              Fields = helpEmbedPages.GetValueOrDefault("default").Select((item, index) =>
                                                    new Embed.Field(item.Name.Replace("{prefix}", ctx.DefaultPrefix), item.Value.Replace("{prefix}", ctx.DefaultPrefix))).ToArray() } },
    ...
    public static Task ButtonClick(InteractionContext ctx, string prefix)
    {
        ...
        if (ctx.Event.Message.Components.First().Components.Where(x => x.CustomId == ctx.CustomId).First().Style == ButtonStyle.Primary)
            ...
            {
                Embeds = new[] { helpEmbed with { Fields = helpEmbedPages.GetValueOrDefault("default").Select((item, index) =>
