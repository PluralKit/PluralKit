...
public class MemberAvatar
{
    ...
    private async Task AvatarClear(MemberAvatarLocation location, Context ctx, PKMember target, MemberGuildSettings? mgs)
    {
        ...
        else if (location == MemberAvatarLocation.MemberWebhook)
        {
            if (mgs?.AvatarUrl != null)
                await ctx.Reply(
                    $"{Emojis.Success} Member proxy avatar cleared. Note that this member has a server-specific avatar set here, type `pk;member {await target.Reference(ctx)} serveravatar clear` if you wish to clear that too.");
            else
        ...
        else
        {
            if (mgs?.AvatarUrl != null)
                await ctx.Reply(
                    $"{Emojis.Success} Member avatar cleared. Note that this member has a server-specific avatar set here, type `pk;member {await target.Reference(ctx)} serveravatar clear` if you wish to clear that too.");
            else
                                  ...
                                  MemberGuildSettings? guildData)
    {
        ...
        var canAccess = location == MemberAvatarLocation.Server ||
                        target.AvatarPrivacy.CanAccess(await ctx.DirectLookupContextFor(target.System));
        ...
        if (string.IsNullOrEmpty(currentValue) || !canAccess)
        {
            ...
            if (location == MemberAvatarLocation.MemberWebhook)
                throw new PKError(
                    $"This member does not have a proxy avatar set. Type `pk;member {target.Reference(lctx)} avatar` to see their global avatar.");

            if (location == MemberAvatarLocation.Server)
                throw new PKError(
                    $"This member does not have a server avatar set. Type `pk;member {target.Reference(lctx)} avatar` to see their global avatar.");
        }
        ...
        var eb = new EmbedBuilder()
            .Title($"{target.NameFor(lctx)}'s {field}")
            .Image(new Embed.EmbedImage(currentValue?.TryGetCleanCdnUrl()));
        if (target.System == ctx.System?.Id)
            eb.Description($"To clear, use `pk;member {target.Reference(lctx)} {location.Command()} clear`.");
