...
public class MemberProxy
{
    public async Task Proxy(Context ctx, PKMember target)
    {
        ...
        async Task<bool> WarnOnConflict(ProxyTag newTag)
        {
            ...
            var conflictList = conflicts.Select(m => $"- **{m.NameFor(lctx)}**");
        ...
        else if (ctx.Match("add", "append"))
        {
            ...
            if (tagToAdd.IsEmpty) throw Errors.EmptyProxyTags(target, lctx);
        ...
        else if (ctx.Match("remove", "delete"))
        {
            ...
            if (tagToRemove.IsEmpty) throw Errors.EmptyProxyTags(target, lctx);
        ...
        else
        {
            ...
            if (requestedTag.IsEmpty) throw Errors.EmptyProxyTags(target, lctx);
