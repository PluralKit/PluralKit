        ...
        public async Task SetLogCleanup(Context ctx)
        {
            ...
            else
            {
                ...
                if (guildCfg.LogCleanupEnabled)
                    eb.Description($"Log cleanup is currently **on** for this server. To disable it, type `{ctx.CommandPrefix}logclean off`."); 
                else 
                    eb.Description($"Log cleanup is currently **off** for this server. To enable it, type `{ctx.CommandPrefix}logclean on`.");
