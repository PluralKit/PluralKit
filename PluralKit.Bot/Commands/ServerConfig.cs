...
public class ServerConfig
{
    ...
    public async Task SetLogEnabled(Context ctx, bool enable)
    {
        ...
        await ctx.Reply(
            ...
            (logChannel == null
                ? $"\n{Emojis.Warn} Please note that no logging channel is set, so there is nowhere to log messages to. You can set a logging channel using `{ctx.DefaultPrefix}log channel #your-log-channel`."
                : ""));
    }
    ...
    public async Task SetLogCleanup(Context ctx)
    {
        ...
        if (newValue == null)
        {
            ...
            if (guildCfg.LogCleanupEnabled)
                eb.Description(
                    $"Log cleanup is currently **on** for this server. To disable it, type `{ctx.DefaultPrefix}logclean off`.");
            else
                eb.Description(
                    $"Log cleanup is currently **off** for this server. To enable it, type `{ctx.DefaultPrefix}logclean on`.");
            await ctx.Reply(embed: eb.Build());
            return;
        }
