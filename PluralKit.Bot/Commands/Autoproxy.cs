            ...
            switch (ctx.MessageContext.AutoproxyMode) {
                ...
                case AutoproxyMode.Front:
                {
                    if (fronters.Length == 0)
                        eb.WithDescription($"Autoproxy is currently set to **front mode** in this server, but there are currently no fronters registered. Use the `{ctx.CommandPrefix}switch` command to log a switch.");
                    else
                    {
                        ...
                        eb.WithDescription($"Autoproxy is currently set to **front mode** in this server. The current (first) fronter is **{relevantMember.NameFor(ctx).EscapeMarkdown()}** (`{relevantMember.Hid}`). To disable, type `{ctx.CommandPrefix}autoproxy off`.");
                    }
                ...
                case AutoproxyMode.Member when relevantMember != null: {
                    eb.WithDescription($"Autoproxy is active for member **{relevantMember.NameFor(ctx)}** (`{relevantMember.Hid}`) in this server. To disable, type `{ctx.CommandPrefix}autoproxy off`.");
                    break;
                }
                case AutoproxyMode.Latch:
                    eb.WithDescription($"Autoproxy is currently set to **latch mode**, meaning the *last-proxied member* will be autoproxied. To disable, type `{ctx.CommandPrefix}autoproxy off`.");
                    break;
