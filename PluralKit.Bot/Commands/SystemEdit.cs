...
namespace PluralKit.Bot
{
    public class SystemEdit
    {
            ...
            if (newDescription == null)
            {
                if (ctx.System.Description == null)
                    ...
                    await ctx.Reply(embed: new EmbedBuilder()
                        ...
                        .Footer(new($"To print the description with formatting, type `{ctx.CommandPrefix}s description -raw`. To clear it, type `{ctx.CommandPrefix}s description -clear`. To change it, type `{ctx.CommandPrefix}s description <new description>`."))
                        .Build());
            }
            else
            {
        ...
        public async Task Color(Context ctx) {
            ...
            else if (!ctx.HasNext()) 
            {
                if (ctx.System.Color == null) 
                    await ctx.Reply(
                            $"Your system does not have a color set. To set one, type `{ctx.CommandPrefix}system color <color>`.");
                else
                    await ctx.Reply(embed: new EmbedBuilder()
                        .Title("System color")
                        .Color(ctx.System.Color.ToDiscordColor())
                        .Thumbnail(new($"https://fakeimg.pl/256x256/{ctx.System.Color}/?text=%20"))
                        .Description($"Your system's color is **#{ctx.System.Color}**. To clear it, type `{ctx.CommandPrefix}s color -clear`.")
                        .Build());
            }
            else
            {
            ...
            async Task ShowIcon()
            {
                if ((ctx.System.AvatarUrl?.Trim() ?? "").Length > 0)
                {
                    var eb = new EmbedBuilder()
                        ...
                        .Description($"To clear, use `{ctx.CommandPrefix}system icon clear`.");
        ...
        {
            if (ctx.System == null) throw Errors.NoSystemError(ctx);
