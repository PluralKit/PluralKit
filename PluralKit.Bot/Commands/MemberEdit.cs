...
namespace PluralKit.Bot
{
    public class MemberEdit
    {
        ...
        public async Task MemberAutoproxy(Context ctx, PKMember target)
        {
            ...
            if (ctx.Match("on", "enabled", "true", "yes") || ctx.MatchFlag("on", "enabled", "true", "yes")) newValue = false;
            else if (ctx.Match("off", "disabled", "false", "no") || ctx.MatchFlag("off", "disabled", "false", "no")) newValue = true;
            else if (ctx.HasNext()) throw new PKSyntaxError("You must pass either \"on\" or \"off\".");
            else
            {
