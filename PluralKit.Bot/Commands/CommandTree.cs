...
public class CommandTree
{
    ...
    private async Task HandleMemberCommandTargeted(Context ctx, PKMember target)
    {
        ...
        if (ctx.Match("rename", "name", "changename", "setname"))
            ...
            if (ctx.Match("add", "a"))
                await ctx.Execute<MemberGroup>(MemberGroupAdd,
                    m => m.AddRemove(ctx, target, Groups.AddRemoveOperation.Add));
            else if (ctx.Match("remove", "rem"))
                await ctx.Execute<MemberGroup>(MemberGroupRemove,
                    m => m.AddRemove(ctx, target, Groups.AddRemoveOperation.Remove));
            else
