            ...
            await ctx.BusyIndicator(async () =>
            {
                using (var client = new HttpClient())
                {
                    ...
                    else if (ctx.System == null)
                    {
                        ...
                        await ctx.Reply($"{Emojis.Success} PluralKit has created a system for you based on the given file. Your system ID is `{result.System.Hid}`. Type `{ctx.CommandPrefix}system` for more information.");
                    }
                    else
                    {
                        // We already had a system, so show them what changed
                        await ctx.Reply($"{Emojis.Success} Updated {result.ModifiedNames.Count} members, created {result.AddedNames.Count} members. Type `{ctx.CommandPrefix}system list` to check!");
                    }
                }
            });
        }
