...
# Commands
## System commands
*To target a specific system, replace `[system]` with that systems' 5-character ID, a Discord account ID, or an @mention - note that system names can not be used here. If no system ID is specified, defaults to targeting your own system. For most commands, adding `-clear` will clear/delete the field.*
